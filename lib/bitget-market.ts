...
export async function getMarket(asset:Asset){
 ...
 async function read(endpoint:string){const response=await fetch(`https://api.bitget.com/api/v3/market/${endpoint}?category=SPOT&symbol=${symbol}${endpoint==='orderbook'?'&limit=20':''}`,{signal:AbortSignal.timeout(10000)});if(!response.ok)throw Error('Market source unavailable.');const body=await response.json() as {code:string;data:unknown};if(body.code!=='00000')throw Error('Market source unavailable.');return body.data;}
